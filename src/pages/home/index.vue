...
<template>
  <div class="space-y-24">
    ...
    <section
      ...
      <!-- Video Background -->
      <video autoplay loop muted playsinline class="fixed top-0 left-0 w-screen h-screen object-cover z-[-1]">
      ...
      <!-- Avatar -->
      <Motion :initial="{ opacity: 0, y: -40, scale: 0.8 }" :animate="{ opacity: 1, y: 0, scale: 1 }"
        :transition="{ duration: 0.7, easing: [0.22, 1, 0.36, 1] }">
        <div class="relative inline-block avatar-wrapper">
          <div class="avatar-ring" />
          <img src="/profile.jpg" alt="Her Bunheng - Full Stack Developer Profile"
      ...
      <Motion :initial="{ opacity: 0, y: 30 }" :animate="{ opacity: 1, y: 0 }"
        ...
        <div class="space-y-3 relative">
          <h1 class="text-4xl sm:text-5xl md:text-7xl font-black tracking-tight leading-none">
          ...
          <div class="flex justify-center">
            <p class="typing-effect text-sm sm:text-base md:text-lg font-semibold text-purple-400">
              {{ t("home.hero.stack") }}
            </p>
          </div>
          <p class="max-w-md sm:max-w-lg md:max-w-xl mx-auto text-slate-400 leading-relaxed text-sm sm:text-base pt-1">
            {{ infor.infomation }}
          </p>
        </div>
      </Motion>
      ...
      <!-- Scroll indicator -->
      <Motion :initial="{ opacity: 0 }" :animate="{ opacity: 1 }" :transition="{ duration: 1, delay: 1 }">
    ...
    <!-- ──────────────── ABOUT ──────────────── -->
    <section id="about" class="grid md:grid-cols-5 gap-8 sm:gap-12 items-center reveal fade-up">
      <!-- Image -->
      <div class="relative group order-1 md:order-2 md:col-span-2">
        ...
        <div
          ...
          <img src="/48hr.jpg" alt="Her Bunheng Portrait - Software Developer"
      ...
      <div class="space-y-4 sm:space-y-6 order-2 md:order-1 md:col-span-3">
        <h2 class="text-3xl sm:text-4xl font-black flex items-center gap-3">
          <span class="icon-badge"><i class="fa-solid fa-user text-purple-400" /></span>
          {{ t("home.about.title") }}
        </h2>
    ...
    <section id="skills" class="space-y-12">
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
        <div v-for="(skill, i) in skills" :key="i"
          ...
          <i :class="skill.icon" class="text-3xl sm:text-4xl block skill-icon" />
          <h3 class="font-bold text-lg sm:text-xl">{{ skill.title }}</h3>
          <ul class="space-y-2 text-slate-400 text-xs sm:text-sm">
            <li v-for="item in skill.items" :key="item" class="flex items-center gap-2">
              <div class="w-1.5 h-1.5 rounded-full shrink-0" :class="skill.dotColor" />
              {{ item }}
            </li>
          </ul>
        </div>
    ...
    <section class="space-y-12">
      ...
      <div class="relative max-w-3xl mx-auto pl-6 sm:pl-8 border-l border-white/10 space-y-12">
        <div v-for="(exp, i) in experience" :key="i" class="relative reveal fade-left"
          :style="{ transitionDelay: `${i * 120}ms` }">
          <!-- Timeline dot with pulse for current -->
          <div class="timeline-dot" :class="exp.current ? 'timeline-dot-active' : 'timeline-dot-inactive'
            ">
          ...
          <div
            ...
            <span class="text-xs sm:text-sm font-bold" :class="exp.current ? 'text-purple-400' : 'text-slate-500'">
              {{ exp.period }}
            </span>
    ...
    <!-- ──────────────── CONTACT ──────────────── -->
    <section id="contact" class="grid md:grid-cols-2 gap-12 pt-12 border-t border-white/10">
      <!-- Info -->
      <div class="space-y-8 reveal fade-up">
        ...
        <div class="space-y-4">
          <div v-for="(info, i) in contactInfo" :key="i" class="contact-info-item reveal fade-left"
        ...
        <div class="flex gap-3">
          <a target="_blank" rel="noopener noreferrer" v-for="social in [
            {
      ...
      <!-- Form -->
      <div class="glass-card border border-white/10 p-8 rounded-2xl shadow-2xl reveal fade-up"
        ...
        <form class="space-y-5" @submit.prevent="handleSubmit">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              ...
              <input type="text" :placeholder="t('home.contact.form.placeholders.name')" class="input-field"
                v-model="form.name" />
            </div>
            <div class="space-y-2">
              <label class="input-label">{{
              ...
              <input type="text" :placeholder="t('home.contact.form.placeholders.email')" class="input-field"
                v-model="form.email" @input="handleEmailInput" />
            </div>
          </div>
          <div class="space-y-2">
            <label class="input-label">{{
          ...
          <div class="space-y-2">
            ...
            <textarea v-model="form.message" :placeholder="t('home.contact.form.placeholders.message')" rows="4"
